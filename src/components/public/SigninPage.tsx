...
import { Loader2, Mail, Lock, AlertCircle, ArrowRight, Eye, EyeOff, Info, ArrowLeft } from "lucide-react";
    ...
    <div className="h-screen w-full flex flex-col items-center justify-center bg-[#020804] text-emerald-100 p-4 sm:p-8 overflow-hidden relative selection:bg-emerald-600/35 selection:text-emerald-100 z-0">
      ...
      <div className="w-full max-w-[420px] sm:max-w-xl md:max-w-2xl relative z-10 flex flex-col justify-center h-full max-h-[900px] my-auto">
        ...
        <div className="bg-[#041008] border border-emerald-800/40 rounded-[20px] p-6 sm:p-10 shadow-[0_20px_50px_rgba(0,0,0,0.8)] relative overflow-hidden animate-in fade-in slide-in-from-bottom-8 duration-700 delay-100">
          ...
          <form onSubmit={handleSubmit} className="space-y-5 relative z-10">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between ml-1">
                <label className="text-[13px] font-medium text-emerald-300/90" htmlFor="password">
                  Password
                </label>
                <Link href="/forgot-password" className="text-[12px] font-medium text-emerald-400/60 hover:text-emerald-300 transition-colors">
                  Forgot password?
                </Link>
              </div>
              <div className="relative group">
              ...
              <div className="relative z-10 flex items-center gap-2">
                {isLoading ? (
                  <>
                    ...
                    <span className="text-emerald-50">Signing in...</span>
                  </>
                ) : (
                  <>
                    <span className="text-base tracking-wide text-emerald-50">Sign In</span>
          ...
          {/* System Notices */}
          <div className="mt-5 pt-4 border-t border-emerald-800/30 relative z-10">
            <div className="flex items-start gap-3 bg-emerald-500/[0.03] border border-emerald-500/15 p-3.5 rounded-xl shadow-inner">
              <Info className="w-4 h-4 shrink-0 text-emerald-400 mt-0.5" />
              <div className="space-y-2 text-[11.5px] text-emerald-200/70 leading-relaxed font-light">
                <p>
                  <strong className="text-emerald-200 font-medium">Server Wake-up:</strong> Our backend runs on a free instance that sleeps during inactivity. We choose to respect our provider's resources rather than artificially keeping it awake, so your initial request may take a few extra seconds to process.
                </p>
                <p>
                  <strong className="text-emerald-200 font-medium">Test Access:</strong> Credentials have been pre-filled for your convenience. Please note that all platform activity is actively monitored by administrators for quality and security.
                </p>
              </div>
            </div>
          </div>
        ...
        <p className="mt-6 text-center text-[15px] text-emerald-200/60 font-light">
          ...
          <Link href="/signup" className="text-emerald-400 font-semibold hover:text-emerald-300 hover:underline underline-offset-4 decoration-emerald-400/40 transition-all">
            Sign up for free
          </Link>
        </p>
      </div>
